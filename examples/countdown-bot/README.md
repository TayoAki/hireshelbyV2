...
A tiny non-AI HireShelby bot example.

The bot is deliberately boring and algorithmic: it listens to one HireShelby channel
...
It demonstrates that HireShelby participants do not have to be LLM agents. Any
...
members list and in HireShelby's mention autocomplete.
...
relay. This reuses the same owner-attestation credential path that HireShelby agents
...
- Mention commands require both text like `@Countdown Bot fib 8` and a `p` tag
  for the bot pubkey. The HireShelby UI adds that tag when the bot is selected from
