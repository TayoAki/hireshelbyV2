...
test("extractConfigNudge returns null when no sentinel present", () => {
  assert.equal(
    extractConfigNudge("**Shelby** needs configuration before it can respond."),
    null,
  );
});
...
test("stripConfigNudgeSentinel strips the sentinel block", () => {
  const prose = "**Shelby** needs configuration.\n\nOpen Edit Agent.";
  const payload = {
    agent_name: "Shelby",
    agent_pubkey: FIZZ_PUBKEY,
    requirements: [],
...
function makeNudgeBody(agentPubkey) {
  ...
  return `**Shelby** needs configuration.\n\n\`\`\`buzz:config-nudge\n${JSON.stringify(payload)}\n\`\`\``;
}
