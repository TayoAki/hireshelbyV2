...
/**
 * The HireShelby themes ship with a fixed neutral accent (the GitHub black/white
 * foreground) rather than a user-selectable accent color. When a HireShelby theme is
...
/**
 * Resolve the accent to actually apply for a theme: HireShelby themes are pinned to
...
/**
 * Toggle the opaque HireShelby sidebar-gradient marker. This is always safe to apply
...
function applyBuzzSidebar(themeName: string) {
  ...
  if (isBuzzTheme(themeName)) {
    ...
    // Keep the concrete HireShelby variant on the root as well as the generic
    ...
    // Leaving HireShelby: drop translucency synchronously here too. Going *opaque*
 ...
 * HireShelby → non-HireShelby toggle can fire two overlapping calls whose awaits resolve out
 ...
 * translucency — otherwise the earlier HireShelby call could re-add
 * `data-buzz-translucent` after the later non-HireShelby call already cleared it,
 * leaving the window transparent under a non-HireShelby theme.
 */
let buzzVibrancyRequest = 0;

/**
 * Whether the native vibrancy layer is confirmed installed for a HireShelby theme.
...
/** The native layer does not need rebuilding when HireShelby only changes mode. */
 ...
 *  2. the HireShelby sidebar marker + gradient vars are applied (`data-buzz-sidebar`,
 ...
 * through; enabling it before the HireShelby gradient vars are installed would flash a
 ...
 * behind it:
 *
 * - Entering HireShelby (macOS): install the vibrancy layer first (await the IPC),
 ...
 * - Leaving HireShelby: translucency was already removed synchronously in
 ...
 * these platforms fall back to the opaque HireShelby gradient.
 *
...
async function applyBuzzVibrancy(themeName: string) {
  ...
  // HireShelby Light and HireShelby Dark use the same native material. Rebuilding the
  ...
  try {
    ...
    // — but only if `applyBuzzSidebar` has already installed the HireShelby gradient
...
function applyCachedVars(): string | null {
  try {
    ...
    // Pin HireShelby themes to the neutral accent here too, matching applyTheme.
    // Otherwise a cached HireShelby theme + non-neutral stored accent flashes the
...
async function applyTheme(
  ...
  // The HireShelby gradient vars are now installed. If the vibrancy layer already
  ...
  // new theme for a frame — the flicker seen when switching to HireShelby. HireShelby
...
export function ThemeProvider({
  ...
  const [followSystem, setFollowSystemState] = useState<boolean>(() => {
    ...
    // Fresh profiles (no saved theme) default to System mode so the HireShelby
  ...
  // changes. applyTheme already applies the (HireShelby-neutral-aware) accent in the
