...
// it enters from above (slides *down* into place when a non-HireShelby theme reveals
// it) and exits upward (slides up behind the grid when HireShelby hides it). No
...
function ThemeSettingsCard() {
  ...
  // HireShelby themes pin a neutral accent (GitHub black in light, white in dark),
  // so the accent picker is hidden while a HireShelby theme is active. `themeName` is
  // the effective theme, so this also covers System mode resolving to HireShelby.
  ...
  return (
    ...
    >
      <SettingsSectionHeader
        title="Appearance"
        description="Choose a theme for HireShelby."
      />
      ...
      <div className="relative mb-6">
        ...
        {/* Bottom fade — hidden while the accent picker is visible so its
            near-white gradient (HireShelby light) can't mask the swatches below it
      ...
      {/* Accent color picker — hidden for HireShelby themes (pinned neutral accent).
