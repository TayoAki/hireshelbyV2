...
export function HostedCommunitiesSettingsCard() {
  ...
  const loadAccount = React.useCallback(async () => {
    ...
    if (
      ...
      // HireShelby identity yet — that's the connect-card empty state, not an error to
      ...
      if (response.error) {
        throw new Error(
          errorMessage(
            ...
            "Could not connect the HireShelby identity.",
          ),
        );
      }
      ...
      if (response.error) {
        throw new Error(
          errorMessage(
            response.error,
            response.correlation_id,
            "Could not unpair the HireShelby identity.",
          ),
        );
      }
      setIdentity(null);
      ...
      if (released.error) {
        throw new Error(
          errorMessage(
            ...
            "Could not release the previously connected HireShelby identity.",
          ),
        );
      }
        ...
        throw new Error(
          bound.error.code === "pubkey_already_bound"
            ...
            : errorMessage(
                ...
                "Could not connect this device's HireShelby identity.",
              ),
        );
      }
      ...
      if (availabilityResponse.error || !availabilityResponse.available) {
        ...
        throw new Error(
          errorMessage(
            ...
            "That HireShelby address is already taken.",
          ),
        );
      }
      const response = await invoke<CommunityMutationResponse>(
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-border/70 p-5">
          ...
          <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
            ...
            HireShelby. You can use every other part of the app without signing
            in.
          </p>
          <Button
            className="mt-4"
          ...
          {!identity ? (
            <div className="rounded-xl border border-amber-500/40 bg-amber-500/5 p-5">
              ...
              >
                ...
                {action ?? "Connect HireShelby identity"}
              </Button>
            </div>
          ) : identityMismatch ? (
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-border/70 p-4">
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                <CheckCircle2 className="h-4 w-4 text-emerald-500" /> HireShelby
                identity connected
                {identity.npub ? (
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            ...
            member. The recipient needs a connected HireShelby identity first,
