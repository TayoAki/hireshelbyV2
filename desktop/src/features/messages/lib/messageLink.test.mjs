...
test("buildMessageLink → parseMessageLink round-trips without thread", () => {
  ...
  assert.equal(url, `hireshelby://message?channel=${CHANNEL}&id=${MESSAGE}`);
...
test("buildMessageLink treats null/empty thread as absent", () => {
  ...
  assert.equal(a, `hireshelby://message?channel=${CHANNEL}&id=${MESSAGE}`);
  assert.equal(b, `hireshelby://message?channel=${CHANNEL}&id=${MESSAGE}`);
});
...
test("parseMessageLink rejects hireshelby:// with wrong host", () => {
  const r = parseMessageLink(`hireshelby://connect?relay=wss://example.com`);
  assert.equal(r.ok, false);
  assert.equal(r.ok === false && r.reason, "wrong-host");
});

test("parseMessageLink rejects missing channel", () => {
  const r = parseMessageLink(`hireshelby://message?id=${MESSAGE}`);
  assert.equal(r.ok, false);
  assert.equal(r.ok === false && r.reason, "missing-channel");
});

test("parseMessageLink rejects missing id", () => {
  const r = parseMessageLink(`hireshelby://message?channel=${CHANNEL}`);
  assert.equal(r.ok, false);
...
test("parseMessageLink accepts legacy hireshelby://message links", () => {
  const r = parseMessageLink(
    `hireshelby://message?channel=${CHANNEL}&id=${MESSAGE}`,
  );
...
test("isMessageLink matches hireshelby://message and legacy hireshelby://message", () => {
  assert.equal(
    isMessageLink(`hireshelby://message?channel=${CHANNEL}&id=${MESSAGE}`),
    true,
  );
  assert.equal(
    isMessageLink(`hireshelby://message?channel=${CHANNEL}&id=${MESSAGE}`),
  ...
  assert.equal(isMessageLink("hireshelby://connect?relay=wss://x"), false);
  assert.equal(isMessageLink("hireshelby://connect?relay=wss://x"), false);
...
test("resolveMessageLinkRenderTarget distinguishes autolinks from labeled links", () => {
  const href = `hireshelby://message?channel=${CHANNEL}&id=${MESSAGE}`;
