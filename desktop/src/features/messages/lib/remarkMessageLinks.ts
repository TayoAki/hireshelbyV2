/**
 * Remark plugin that detects bare `hireshelby://message?…` URLs in text nodes and
 * replaces each with a custom `message-link` HAST element. Legacy
 * `hireshelby://message?…` URLs are accepted during the rename. The `markdown.tsx`
 ...
 * and `www.`. Custom schemes like `hireshelby://` only reach the `<a>` component
 * override when the user wrote an explicit `[label](hireshelby://…)` link.
 *
...
const MESSAGE_URL_PATTERN = /hireshelby:\/\/message\?[^\s<>"')\]]+/g;
