...
test("HireShelby shared compute maps persisted auto to Default and hides custom input", () => {
...
test("HireShelby shared compute fallback is Default auto while normal providers remain unchanged", () => {
...
test("HireShelby shared compute keeps Default auto when discovery is empty", () => {
  const state = relayMeshModelPickerState({
    discoveredOptions: [],
    fallbackOptions: fallback,
    isCustomEditing: false,
...
test("HireShelby shared compute dropdown contains Default plus live models and no custom escape hatch", () => {
