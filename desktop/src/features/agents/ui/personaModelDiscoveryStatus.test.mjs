...
test("HireShelby shared compute names the empty state and next action", () => {
  const status = formatModelDiscoveryErrorStatus(
    new Error("no HireShelby shared compute serving members are available"),
    "relay-mesh",
  );
...
test("HireShelby shared compute distinguishes relay lookup failures", () => {
  const status = formatModelDiscoveryErrorStatus(
    new Error(
      "HireShelby shared compute model discovery failed: relay offline",
    ),
    "relay-mesh",
  );
...
test("HireShelby shared compute names a missing relay member roster", () => {
  const status = formatModelDiscoveryErrorStatus(
    new Error(
      "HireShelby shared compute is waiting for the current member roster",
    ),
    "relay-mesh",
  );
