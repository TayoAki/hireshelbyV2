...
test("parsePromptText wraps header-less free text in a single Prompt section", () => {
  ...
  // section is a "HireShelby event", there is no event content to surface, so
  // userText is empty and the title falls through to "HireShelby event".
  ...
  assert.equal(result.userTitle, "HireShelby event");
...
test("parsePromptText extracts event id, content, hex pubkey, and a title-cased kind", () => {
  ...
  assert.deepEqual(
    result.sections.map((s) => s.title),
    ["System", "HireShelby event: @mention"],
  );
});
...
test("parsePromptText defaults the title to 'HireShelby event' when no kind is present", () => {
  const text = ["[HireShelby event]", "Content: x"].join("\n");
  const result = parsePromptText(text);
  assert.equal(result.userTitle, "HireShelby event");
});
...
test("extractToolIdentity ignores HireShelby tool names that only appear in file contents", () => {
...
test("extractToolIdentity still recognizes explicit HireShelby tool fields", () => {
  const identity = extractToolIdentity({
...
test("parseSystemPromptSections pins the realistic Workspace+Base+System+Core harness shape", () => {
  // The real HireShelby harness emits [Workspace] content before [Base]. The parser
  ...
  const framed = [
    "[Workspace]",
    "You are operating inside the HireShelby platform.",
    "",
    "[Base]",
...
test("parseSystemPromptSections extracts Team Instructions with Core Memory and Channel Canvas (full 5-section shape)", () => {
  ...
  const framed = [
    "[Base]",
    "You are a helpful AI assistant running in HireShelby.",
    "",
    "[System]",
  ...
  assert.deepEqual(sections, [
    {
      title: "Base",
      body: "You are a helpful AI assistant running in HireShelby.",
    },
    {
      title: "System",
