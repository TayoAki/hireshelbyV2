...
test.describe("config bridge screenshots", () => {
  ...
  test("01 — folded config panel", async ({ page }) => {
    ...
    await expect(panel.getByText("Set in HireShelby").first()).toBeVisible();
  ...
  test("06 — buzz-agent empty MCP servers", async ({ page }) => {
    ...
    const panel = await openAgentProfileFromChannel(page, "HireShelby Agent");
