...
async function chooseSharedComputeProvider(
  ...
  await page
    .getByRole("menuitemradio", {
      exact: true,
      name: "HireShelby shared compute",
    })
    .click();
}
...
test("HireShelby shared compute explains automatic model selection", async ({
  page,
}) => {
  await page.goto("/");
  await page.evaluate(() => {
...
test("create agent persists HireShelby shared compute with auto model", async ({
  page,
}) => {
