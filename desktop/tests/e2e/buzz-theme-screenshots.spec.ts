...
test("appearance picker — system tab (HireShelby follows OS)", async ({
  page,
}) => {
  await seedTheme(page, "buzz");
  await installMockBridge(page);
...
test("appearance picker — light tab (HireShelby)", async ({ page }) => {
  await seedTheme(page, "buzz");
  await installMockBridge(page);
...
test("appearance picker — dark tab (HireShelby Dark)", async ({ page }) => {
  await seedTheme(page, "buzz-dark");
  await installMockBridge(page);
...
test("settings nav uses HireShelby active pill + hover (light)", async ({
  page,
}) => {
  await seedTheme(page, "buzz");
  await installMockBridge(page);
  ...
  // Appearance is the active section here; its nav row should carry the HireShelby
...
test("settings nav uses HireShelby active pill + hover (dark)", async ({
  page,
}) => {
  await seedTheme(page, "buzz-dark");
  await installMockBridge(page);
...
test("appearance hides accent picker under HireShelby", async ({ page }) => {
  await seedTheme(page, "buzz");
  await installMockBridge(page);
  const panel = await openAppearance(page, "light");
  // The accent picker is hidden while a HireShelby theme is active. Its neutral
...
test("accent picker reveals/hides when toggling HireShelby", async ({
  page,
}) => {
  // Start on a non-HireShelby theme so the accent picker is present, then select the
  // HireShelby tile — the picker should animate out and unmount. Reselecting a
  // non-HireShelby tile brings it back. Asserts the presence toggle (the motion
  ...
  // Switch to HireShelby — picker should leave (allow the exit animation to settle).
  await page.getByTestId("theme-option-buzz").click();
  await expect(page.getByTestId("accent-color-neutral")).toHaveCount(0);

  // Back to a non-HireShelby theme — picker returns.
  await page.getByTestId("theme-option-github-light").click();
  await expect(page.getByTestId("accent-color-neutral")).toBeVisible();
});

test("HireShelby light and dark modes apply live without a reload", async ({
  page,
}) => {
  await seedTheme(page, "buzz");
  await installMockBridge(page);
...
test("HireShelby follows native system theme changes without a reload", async ({
  page,
}) => {
  await seedTheme(page, "buzz");
  await page.addInitScript(() => {
