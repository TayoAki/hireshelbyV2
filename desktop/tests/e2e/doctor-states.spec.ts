...
const CODEX_NOT_INSTALLED = {
  ...
  default_args: [],
  mcp_command: null,
  install_hint: "HireShelby talks to Codex through the Codex CLI.",
...
test.describe("Doctor panel state screenshots", () => {
  ...
  test("00-runtime-card-layout", async ({ page }) => {
    ...
    ).toContainText("HireShelby talks to Codex through the Codex CLI.");
    await expect(
      page
        ...
        },
        {
          ...
          steps: [
            ...
            },
            {
              ...
              stdout: "",
              stderr:
                "The installer finished, but HireShelby still could not use codex (observed: NotInstalled).",
              exit_code: null,
              hint: "HireShelby requires the vendor CLI executable, not only its desktop app. If the CLI was installed while HireShelby was open, restart HireShelby and check again.",
            },
          ],
        },
      ],
  ...
  test("11-outdated-adapter-warning", async ({ page }) => {
    ...
    await expect(dialog).toContainText(
      "Older HireShelby releases using the legacy adapter may lose community access",
    );
