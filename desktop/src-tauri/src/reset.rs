...
/// (e.g. `com.hireshelby.app` or `com.hireshelby.app.dev`).
...
mod tests {
    ...
    fn test_crash_retry_cleans_prior_deterministic_trash() {
        ...
        let app_data = app_support.join("com.hireshelby.app");
        std::fs::create_dir_all(&app_data).unwrap();
        write_sentinel(&app_data).unwrap();
        ...
        let trash_app_dir = app_support.join("com.hireshelby.app.reset-trash");
