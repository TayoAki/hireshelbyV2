...
pub struct AcpAuthMethod {
    ...
    #[serde(default)]
    pub args: Vec<String>,
    /// Full terminal command advertised by the adapter. HireShelby never guesses
...
/// resolve the HireShelby-managed Node runtime — the same PATH normal agent
/// launches and readiness probes use.
///
/// On Windows, `login_shell_path()` is intentionally `None`, so the augmented
/// PATH contains only HireShelby-managed directories and the exe parent. HireShelby does
...
mod tests {
    ...
    /// Windows regression: the augmented PATH there holds only HireShelby-managed
