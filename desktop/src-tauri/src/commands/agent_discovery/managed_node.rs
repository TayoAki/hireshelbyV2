...
fn managed_node_unsupported_step() -> InstallStepResult {
    InstallStepResult {
        ...
        stdout: String::new(),
        stderr: format!(
            "HireShelby does not provide a managed Node.js runtime for {}-{} yet",
            std::env::consts::OS,
            std::env::consts::ARCH
        ),
        exit_code: None,
        hint: Some(
            "Install Node.js from https://nodejs.org, restart HireShelby, then click Install again."
                .to_string(),
        ),
    }
}
...
fn managed_node_install_hint() -> String {
    "HireShelby could not install its private Node.js runtime. Check your network and app-data directory permissions, then click Install again.".to_string()
}
...
pub(super) fn ensure_managed_node_runtime_blocking() -> Result<(), Box<InstallStepResult>> {
    ...
    let Some(root) = crate::managed_agents::buzz_managed_node_root() else {
        return Err(Box::new(managed_node_failed_step(
            "failed to resolve HireShelby app-data directory for private Node.js runtime"
                .to_string(),
        )));
    };
...
/// Guidance text shown when the HireShelby-private npm prefix is not available.
fn managed_npm_prefix_hint() -> String {
    "HireShelby could not create its private Node tools directory. Check app-data directory permissions, restart HireShelby, then click Install again.".to_string()
}
...
pub(super) fn managed_npm_command(command: &str) -> Result<Option<String>, Box<InstallStepResult>> {
    ...
    let Some(prefix) = crate::managed_agents::buzz_managed_npm_prefix() else {
        return Err(Box::new(InstallStepResult {
            ...
            success: false,
            stdout: String::new(),
            stderr: "failed to resolve HireShelby app-data directory for private npm prefix"
                .to_string(),
            exit_code: None,
    ...
    if let Err(error) = std::fs::create_dir_all(&prefix) {
        return Err(Box::new(InstallStepResult {
            ...
            stdout: String::new(),
            stderr: format!(
                "failed to create HireShelby private npm prefix '{}': {error}",
                prefix.display()
            ),
            exit_code: None,
            hint: Some(managed_npm_prefix_hint()),
...
pub(super) fn npm_eacces_hint(stderr: &str, _command: &str) -> Option<String> {
    if stderr.contains("EACCES: permission denied") || stderr.contains("npm error EACCES") {
        Some(
            "npm could not write to HireShelby's private Node tools directory. Check app-data directory permissions, restart HireShelby, then click Install again."
                .to_string(),
        )
    } else {
        None
...
mod tests {
    ...
    fn test_npm_eacces_hint_guidance_mentions_buzz_private_dir() {
        ...
        assert!(
            hint.contains("HireShelby's private Node tools directory"),
            "hint: {hint}"
        );
    }
    ...
    fn test_rewrite_npm_install_uses_private_prefix() {
        assert_eq!(
            ...
            "npm install --global --prefix '/tmp/HireShelby Node' @agentclientprotocol/codex-acp"
        );
    }
    ...
    fn test_shell_quote_escapes_single_quotes() {
        assert_eq!(
            shell_quote(std::path::Path::new("/tmp/HireShelby's Node")),
            "'/tmp/HireShelby'\\''s Node'"
        );
    }
