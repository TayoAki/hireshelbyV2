...
pub async fn discover_agent_models(
    ...
    // HireShelby shared compute discovery must not depend on the local OpenAI ingress: that
        ...
        == Some(crate::managed_agents::RELAY_MESH_PROVIDER_ID)
    {
        ...
        .await
        .map_err(|error| format!("HireShelby shared compute model discovery failed: {error}"))?;
        ...
        if availability.models.is_empty() {
            return Err(availability.reason.unwrap_or_else(|| {
                "No live HireShelby shared compute models are available".to_string()
            }));
        }
    ...
    if input.provider.as_deref().map(str::trim)
        == Some(crate::managed_agents::RELAY_MESH_PROVIDER_ID)
    {
        return Err("HireShelby shared compute is not available in this build".to_string());
    }
