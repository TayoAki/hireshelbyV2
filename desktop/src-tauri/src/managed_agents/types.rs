...
pub struct AgentDefinition {
    ...
    /// HireShelby spawns. When deploying from this persona, this runtime is pre-selected in the UI.
    ...
    /// direct). HireShelby stores and passes through without interpretation.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// Opaque to HireShelby — keys and values are runtime-specific.
    ///
...
pub struct ManagedAgentRecord {
    ...
    /// inference through HireShelby's relay-mesh local endpoint; the `model_ref` is
...
pub struct InstallStepResult {
    ...
    /// recognized condition (e.g. EACCES writing HireShelby's private npm prefix).
