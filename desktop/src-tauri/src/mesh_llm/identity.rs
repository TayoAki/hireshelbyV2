...
//! HireShelby/Nostr identity). The node presents a signed ownership attestation
...
//! has one owner identity whether mesh runs embedded in HireShelby or standalone.
...
impl OwnerIdentity {
    /// Sign a HireShelby-to-MeshLLM ownership binding. The member's Nostr signature
