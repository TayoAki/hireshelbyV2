...
//! GPU/driver/compositor combinations, so HireShelby comes up with no window at all
...
/// process is still single threaded, which it is nowhere else in HireShelby.
///
