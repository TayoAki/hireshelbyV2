#!/usr/bin/env bash
# grab-emoji.sh — Register custom Slack emoji in HireShelby
#
# Looks up each emoji name in your Slack workspace and registers it in HireShelby
# via `buzz emoji set`, making it available as :name: in the HireShelby emoji picker.
#
# Usage:
#   SLACK_TOKEN=xoxp-... ./scripts/grab-emoji.sh [--name <buzz-name>] <emoji-name> [emoji-name ...]
#
# Options:
#   --name <buzz-name>  Override the shortcode used in HireShelby (only valid with a single emoji)
#
# Env:
#   SLACK_TOKEN  — Slack user token (xoxp-...) with emoji:read scope
#
# Output:
#   name → registered as :name: in HireShelby   on success
...
if ! command -v buzz &>/dev/null; then
  echo "ERROR: 'buzz' not found in PATH. Install the HireShelby CLI and retry." >&2
  exit 1
fi
...
for emoji_name in "$@"; do
  ...
  echo "${emoji_name} → registered as :${buzz_shortcode}: in HireShelby"
done
