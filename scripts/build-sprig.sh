#!/usr/bin/env bash
# Build Sprig — one deploy-anywhere multicall binary for the HireShelby ACP
...
Sprig is the all-in-one HireShelby agent binary for deploy-anywhere environments.
...
- `buzz-acp` — ACP harness that bridges HireShelby channel events to an
