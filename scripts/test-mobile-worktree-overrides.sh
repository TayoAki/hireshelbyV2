...
grep -q '^APP_DISPLAY_NAME = HireShelby (Fix_Thing-2)$' "$ios" \
...
grep -Eq "^APP_DISPLAY_NAME = HireShelby \([A-Za-z0-9._-]+\)$" "$ios" \
...
grep -q 'resValue("string", "app_name", "HireShelby")' "$gradle" \
  && pass "Gradle default app_name stays HireShelby" \
