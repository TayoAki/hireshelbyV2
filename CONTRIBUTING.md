# Contributing to HireShelby

Welcome, and thank you for your interest in contributing! HireShelby is an
open-source project and we're glad you're here. This guide will help you
get from zero to a merged pull request.

If you have questions that aren't answered here, [open an issue](https://github.com/hireshelby/hireshelby/issues/new).
...
Before starting, search [open PRs](https://github.com/hireshelby/hireshelby/pulls) and [open issues](https://github.com/hireshelby/hireshelby/issues) for duplicates — someone may already be working on the same thing. When you open your PR, link the closest existing one in the description (or say "none found").
...
HireShelby is an agent platform, so AI-assisted PRs are welcome. No need to disclose the tools you used, but you own and must have reviewed the final code. Submissions that are clearly unreviewed may be closed with a pointer here.
...
```bash
# 1. Clone the repo
git clone https://github.com/hireshelby/hireshelby.git
cd buzz
...
just reset   # Wipe all dev state and recreate it; installed HireShelby is preserved
```

Development desktop state uses separate bundle identifiers
(`com.hireshelby.app.dev` and per-worktree variants), a separate keyring service
...
`com.hireshelby.app` data, `buzz-desktop` keyring service, or `~/.buzz` nest.
...
HireShelby is developed across multiple repositories. This repo (`block/buzz`)
...
HireShelby is licensed under the **Apache License, Version 2.0**. See
...
*Thank you for contributing to HireShelby. Every bug report, documentation fix,
