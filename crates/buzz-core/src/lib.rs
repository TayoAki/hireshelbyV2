#![deny(unsafe_code)]
#![warn(missing_docs)]
//! `buzz-core` — zero-I/O foundation types for the HireShelby relay.
//!
//! Provides [`StoredEvent`], filter matching, kind constants, and event
//! verification. All other HireShelby crates depend on this one.
...
pub mod invite;
/// HireShelby kind number registry — custom event type constants.
pub mod kind;
