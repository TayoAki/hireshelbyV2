//! HireShelby V2 kind number registry.
//!
//! This module is the authoritative source for HireShelby kind numbers.
...
/// NIP-01: Channel metadata (replaceable). Not used by HireShelby today.
...
/// HireShelby custom one-time identity binding proof (ephemeral, not stored).
...
// NEW: HireShelby command kinds (Pure Nostr plan)
...
/// HireShelby product feedback submission. Accepted at ingest, sidecarred to the
...
// HireShelby community moderation commands (mod-signed, processed like 9030-series:
...
/// HireShelby: Set the workspace profile (icon). Admin/owner-signed command.
...
// Not using NIP-90 kinds (5000–6999) — HireShelby requires auth chains (depth ≤ 3, breadth ≤ 10).
...
/// or the HireShelby workspace-profile admin command (9033).
...
/// Returns `true` if `kind` is a HireShelby command kind that requires transactional execution.
...
/// Safe: all HireShelby kinds fit in i32 (max 65535 < i32::MAX).
...
// Compile-time: all HireShelby kind constants fit in nostr's u16-backed Kind.
