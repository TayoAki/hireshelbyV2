    ...
    fn from_event(event: &serde_json::Value) -> Option<Self> {
        ...
        for tag in tags {
            ...
            match key {
                ...
                // NIP-29 emits both `private` and `public` (HireShelby adds the latter).
            ...
            many => {
                ...
                return Err(CliError::Usage(format!(
                    "persona '{slug}' has {} live instances for this owner ({}); \
                     ...
                     the duplicate in HireShelby Desktop before creating the channel",
                    many.len(),
                    candidates.join(", ")
                )));
            }
...
mod tests {
    ...
    fn name_matches_substring_case_insensitive() {
        assert!(name_matches("HireShelby-Chat-Composer", "composer", false));
        assert!(name_matches("HireShelby-Chat-Composer", "buzz", false));
        assert!(!name_matches("design", "composer", false));
    }

    #[test]
    fn name_matches_exact_case_insensitive() {
        assert!(name_matches("HireShelby", "buzz", true));
        assert!(!name_matches("HireShelby-Chat", "buzz", true));
    }
