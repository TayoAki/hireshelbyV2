...
/// HireShelby always generates marker-based tags (see relay messages.rs:762-783).
...
/// checking whether the **first** prompt content block starts with `/`. HireShelby
...
pub fn extract_slash_command(content: &str, known_names: &[&str]) -> Option<String> {
    ...
    loop {
        if rest.starts_with("nostr:npub1") || rest.starts_with("nostr:nprofile1") {
            ...
            // token of the characters HireShelby allows in plain @mentions.
            let name_len = names
                .iter()
                .find_map(|name| {
...
/// 5. `[Event]` / `[HireShelby events]` — the triggering event(s)
///
...
pub fn format_prompt(batch: &FlushBatch, args: &FormatPromptArgs<'_>) -> Vec<String> {
    ...
    let event_section = if batch.events.len() == 1 {
        ...
        let header = if has_cancelled {
            ...
            format!("[HireShelby events — {} events]", batch.events.len())
        };
        let mut s = header;
...
mod tests {
    ...
    fn test_format_prompt_single() {
        ...
        assert!(prompt.contains("[HireShelby event: @mention]\n"));
    ...
    fn test_format_prompt_batch() {
        ...
        assert!(prompt.contains("[Context]"));
        assert!(prompt.contains("[HireShelby events — 3 events]"));
