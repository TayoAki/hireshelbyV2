...
ACP harness that connects AI agents to HireShelby. The harness listens for @mentions on the relay, prompts your agent, and the agent replies using the HireShelby CLI.

```
HireShelby Relay ──WS──→ buzz-acp ──stdio──→ Your Agent
                                               │
                                          HireShelby CLI
                                       (send_message, etc.)
```
...
- A running HireShelby relay (`just relay` starts Docker services automatically, or use a hosted instance)
...
Each agent needs a Nostr keypair — this is the agent's identity in HireShelby. Use `buzz-admin` to generate one:
...
**Private channels** require explicit membership. The relay doesn't yet have a REST/event API for managing channel members — this is a known gap. For now, use `create_channel` via the HireShelby CLI to create new channels (the creator is automatically a member).
...
That's it. The harness spawns `goose acp`, connects to the relay, discovers channels, and starts listening. When someone @mentions the agent, goose receives the message and can reply using the HireShelby CLI that the harness configures automatically.
...
5. **Agent response** — The agent processes the prompt and uses the HireShelby CLI (`send_message`, `get_messages`, etc.) to interact with HireShelby.
...
HireShelby Desktop supports registering any ACP-speaking agent tool as a selectable runtime without a PR.

### How it works

**Tier-1 — compiled-in runtimes** (Goose, Claude Code, Codex, HireShelby Agent): have auto-installers, auth probes, and first-class onboarding. Their IDs (`goose`, `claude`, `codex`, `buzz-agent`) are reserved and cannot be overridden.
...
- `env` — optional environment variables injected at spawn time (definition env is a floor; user/persona/global env overrides it; HireShelby-reserved keys like `BUZZ_MANAGED_AGENT` are always stripped and cannot be overridden)
...
- `BUZZ_MANAGED_AGENT` and other HireShelby identity keys cannot be overridden by `env` in a custom definition; they are stripped before merging.
