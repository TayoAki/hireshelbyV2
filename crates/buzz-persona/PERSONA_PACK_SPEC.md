...
for deployment in HireShelby. It is a **superset of the [Open Plugin Spec](https://open-plugin-spec.org)**
...
1. **Portable** — zip file or git repo; no HireShelby tooling required to inspect
...
and HireShelby-specific extensions live alongside the OPS fields at the top level. Since the Open
...
```json
{
  ...
  "version": "1.2.0",
  "description": "A four-agent security review team for HireShelby.",
...
- **HireShelby**: reads both OPS fields and the HireShelby-specific fields; `personas` is authoritative.
- **Version negotiation**: `engines.buzz` specifies minimum required HireShelby version; buzz-acp
  rejects packs requiring a newer version.
- **Extension mechanism**: HireShelby-specific fields sit at the top level of `plugin.json` alongside
...
# === Behavioral Config (HireShelby-specific) ===
subscribe:
  - "#security-reviews"
  - "#code-reviews"
triggers:
...
| `display_name` | string | ✅ | Human-readable name shown in HireShelby UI. |
...
| Content | Purpose |
|---------|---------|
| Platform identity | Tells the agent it is running inside HireShelby and what that means |
...
Pack authors do not write or configure the `[Base]` layer — it is maintained by the HireShelby team
...
- That the agent is running inside HireShelby (covered by `[Base]`)
...
HireShelby conversations. These are all HireShelby-specific — the agent runtime has no awareness of them. They sit
...
2. Desktop UI per-agent        — overrides set in the HireShelby desktop app per-agent settings
...
The `#` prefix in `subscribe` entries is a **display convention only**. Channel names in the HireShelby
...
A HireShelby-hosted registry and in-app browser for discovering and installing packs. API-compatible
...
The HireShelby desktop app can import persona packs via the Import button:
