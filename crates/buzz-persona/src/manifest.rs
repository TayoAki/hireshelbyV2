...
//! (OPS metadata) and tells HireShelby where to find personas, hooks, and MCP
//! config.
//!
//! ```json
//! {
...
pub struct Engines {
    /// Semver range the HireShelby runtime must satisfy (e.g. `">=0.9.0"`).
...
/// validator issues advisory warnings for HireShelby-unknown keys.
