//! git-credential-nostr — NIP-98 git credential helper for HireShelby.
//!
...
/// Load the NIP-OA owner attestation injected by HireShelby Desktop/ACP.
///
...
pub fn run() -> i32 {
    ...
    // No Nostr challenge from the server — this isn't a HireShelby remote.
    // Exit silently so git falls through to the next credential helper.
    // This check comes FIRST so non-HireShelby remotes never hit validation errors.
