...
/// Normalize a HireShelby display name into a git author name, or `None` to fall
/// back to the npub.
///
...
fn build_git_env(info: &KeyInfo) -> Vec<(String, String)> {
    ...
    let entries: Vec<(&str, String)> = vec![
        // Identity — HireShelby display name (npub fallback), NIP-05-style email
        ("user.name", user_name),
        ("user.email", email),
        // Nostr credential helper is additive — it silently declines non-HireShelby
        ...
        // Required: HireShelby relay verifies NIP-98 against the full repo-root URL.
