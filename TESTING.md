...
> **Already running HireShelby Desktop?** Desktop uses the same Docker container
...
`just reset` wipes all local data and starts over — **including HireShelby
...
> **Already running HireShelby Desktop (or another relay) on `:3000` / `:8080` /
> `:9102`?** HireShelby binds three ports — main, health, metrics — and any of
...
> overrides — or the CLI will end up talking to HireShelby Desktop's relay.
