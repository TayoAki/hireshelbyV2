"""Run the production HireShelby agent stack inside the Harbor task container.
...
PATH) that the desktop app gives a HireShelby agent. The harness stays outside:
...
# Container-side layout for the uploaded HireShelby stack.
...
class RuntimeLaunchError(RuntimeError):
    """Raised when a HireShelby agent process cannot be launched or exits early."""
...
class BuzzContainerRuntime:
    """Launch one production HireShelby agent stack per identity in the container."""
    ...
    async def run(
        ...
        *,
        ...
        if not workers:
            raise RuntimeLaunchError("HireShelby orchestration requires at least one worker")
        ...
        try:
            ...
            # The task arrives exactly as it would in production HireShelby: a
    ...
    # -- HireShelby CLI as the trial user / provisioning identities -------------------
    ...
    def _compose_system_prompt(
        ...
        *,
        ...
        The analogue of a production HireShelby workspace's team context: each agent
        ...
        lines = [
            ...
            f"The team coordinates in HireShelby channel `{trial.channel_id}`.",
            (
