# Harbor HireShelby Orchestra

A stock-Harbor custom agent that runs a manifest-defined team through the real
HireShelby stack. Harbor sees one `BuzzOrchestraAgent`; behind that adapter, one
...
Each trial gets fresh keys and a private HireShelby channel. The provisioner archives
...
relay membership list and opens the HireShelby desktop app logged in as them, so
