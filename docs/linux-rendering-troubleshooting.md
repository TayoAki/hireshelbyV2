...
| Blank window on startup, no crash output | dmabuf renderer incompatibility (NVIDIA or AppImage) | `WEBKIT_DISABLE_DMABUF_RENDERER=1 ./HireShelby.AppImage` or `--safe-rendering` |
...
**Affected distributions:** Fedora 40+ and any distro shipping Google's Noto Color Emoji in COLRv1 format (`Noto-COLRv1.ttf`). Issues [#2548](https://github.com/hireshelby/hireshelby/issues/2548), [#2982](https://github.com/hireshelby/hireshelby/issues/2982).

**Symptom:** HireShelby starts, the window appears briefly (or stays blank), then the process aborts with output like:
...
**Fix:** Upgrade to the latest AppImage (v0.5.2+). The build container was bumped to `ubuntu:24.04` ([#3602](https://github.com/hireshelby/hireshelby/pull/3602)), which ships FreeType 2.13.2. The compiled layout now matches every crash-affected host (FreeType ≥ 2.13), eliminating the ABI mismatch.
...
**Workaround (before upgrading):** Add a fontconfig override that removes color-format fonts from HireShelby's view:
...
**Native packages (`deb`/`rpm`):** The COLRv1 crash ([#2548](https://github.com/hireshelby/hireshelby/issues/2548), [#2982](https://github.com/hireshelby/hireshelby/issues/2982)) is AppImage-only — native packages use the system WebKit, which has a consistent FreeType ABI, and are not affected.

---

## Blank window on startup (no crash): dmabuf renderer

**Affected hardware:** NVIDIA GPUs (proprietary and nouveau drivers) and AppImage installs on any GPU. Issue [#2338](https://github.com/hireshelby/hireshelby/issues/2338).

**Symptom:** HireShelby launches without any crash or assertion output, but the window is blank or invisible. The process is running (`ps aux | grep buzz`), but nothing renders.
...
**Fix (shipped automatically starting with the first release containing [#3271](https://github.com/hireshelby/hireshelby/pull/3271) (v0.5.1)):** HireShelby sets `WEBKIT_DISABLE_DMABUF_RENDERER=1` automatically before WebKit initializes when it detects an NVIDIA GPU (`/sys/class/drm` vendor ID `0x10de`) or when running as an AppImage. This restores a slightly slower shared-memory rendering path that works universally.
...
**Conflict detection:** If you set a WebKit variable in your environment and also pass `--safe-rendering`, HireShelby will refuse to start and print exactly which variable conflicts. Unset the conflicting variable or drop the flag.

---

## AMD RDNA4 / transparent window

**Affected hardware:** AMD RDNA4 GPUs (RX 9000 series) with the `radv` driver. Issue [#2643](https://github.com/hireshelby/hireshelby/issues/2643).

**Symptom:** The HireShelby window is transparent or renders with graphical corruption on AMD RDNA4 hardware.

**Workaround (verified by reporter):** Set these three variables before launching HireShelby:
...
A dedicated fix for RDNA4 detection is being tracked in [#2643](https://github.com/hireshelby/hireshelby/issues/2643).
...
1. Run HireShelby from a terminal and capture the output:
   ```bash
...
4. File a [new issue](https://github.com/hireshelby/hireshelby/issues/new) with your distro, GPU, driver version, and the terminal output.
