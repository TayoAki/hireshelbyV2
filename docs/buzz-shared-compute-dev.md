# HireShelby shared compute: local GUI verification

This runbook verifies the actual desktop path used by the built-in **Fizz** agent:

`HireShelby Desktop → buzz-acp → buzz-agent → MeshLLM SDK → local/remote compute`
...
`~/.buzz-dev`, and local Docker volumes; it preserves the installed HireShelby app's
...
If using `mesh-dev-fresh`, the clean window opens at **Welcome to HireShelby**. Join
the seeded local community before continuing:

1. Click **Join a community**.
2. Use any local name, such as **Local HireShelby**.
...
Stop only stale HireShelby/MeshLLM processes shown by that command. Do not leave a
...
take several minutes. Wait for the HireShelby window to open and for the terminal to
...
HireShelby may download the model on first use. The model picker ranks models for the
...
**buzz-agent** as the runtime, **HireShelby shared compute** as the LLM provider,
...
- no current HireShelby membership snapshot is available (admission fails closed).

## Security boundary

HireShelby publishes member-signed discovery notes through an ordinary relay-supported
...
tokens in the note. Current HireShelby membership controls which node identities are
...
MeshLLM—not the HireShelby relay—carries inference over direct QUIC or its encrypted
...
is an upstream protocol change and is not claimed by the HireShelby-side checks above.
